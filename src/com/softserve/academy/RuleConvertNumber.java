...
public class RuleConvertNumber {
    ...
    public static String convert(long number) {
        ...
        if (number < 0) {
            ...
            result = getEndThousand(convert((number / ONE_THOUSAND)))
                    + (number % ONE_THOUSAND > 0 ? " "
                    + convert(number % ONE_THOUSAND) : "");
        } else if (number < ONE_BILLION) {
            result = getEndMillion(convert((number / ONE_MILLION)))
                    + " "
                    + getEndThousand(convert(((number % ONE_MILLION) / ONE_THOUSAND)))
                    + (number % ONE_THOUSAND > 0 ? " "
                    + convert(number % ONE_THOUSAND) : "");
        } else if (number < ONE_TRILLION) {
            result = getEndBillion(convert((number / ONE_BILLION)))
                    + " "
...
// correct ending for thousands
    ...
    private static String getEndMillion(String str) {
        if (str.equals("ноль")) {
            return "";
        } else if (str.endsWith("один")) {
            return str + " миллион";
    ...
    private static String getEndBillion(String str) {
        if (str.equals("ноль")) {
            return "";
        } else if (str.endsWith("один")) {
            return str + " миллиард";
